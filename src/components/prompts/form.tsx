  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row gap-1">
        <ActionButton text="Sử dụng" onClick={() => props.insertPrompt(props.prompt)} />
        {props.edit && <ActionButton text="Chỉnh sửa" onClick={props.edit} />}
        {props.addToLocal && <ActionButton text={saved ? 'Đã thêm' : 'Thêm'} onClick={copyToLocal} />}
        {props.remove && <ActionButton text="Xóa" onClick={props.remove} />}
      </div>
    </div>
  )
}
  ...
  return (
    <form className="flex flex-col gap-2 w-full" onSubmit={onSubmit}>
      <div className="w-full">
        <span className="text-sm font-semibold block mb-1 text-primary-text">Tiêu đề gợi ý</span>
        <Input className="w-full" name="title" defaultValue={props.initialData.title} />
      </div>
      <div className="w-full">
        <span className="text-sm font-semibold block mb-1 text-primary-text">Nội dung gợi ý</span>
        <Textarea className="w-full" name="prompt" defaultValue={props.initialData.prompt} />
      </div>
      <div className="flex flex-row gap-2 mt-1">
        <Button color="primary" className="w-fit" type="submit">Lưu</Button>
        <Button variant="secondary" className="w-fit" onClick={props.onClose}>Hủy</Button>
      </div>
    </form>
  )
}
    ...
    <>
      {localPrompts.length ? (
        ...
        <div className="relative block w-full rounded-lg border-2 border-dashed border-gray-300 p-3 text-center text-sm mt-5 text-primary-text">
          Bạn chưa có từ gợi ý, vui lòng thêm từ này theo cách thủ công
        </div>
      )}
      <div className="mt-5">
        {formData ? (
          ...
          <Button onClick={create}>Tạo từ nhắc nhở</Button>
        )}
      </div>
    </>
  )
